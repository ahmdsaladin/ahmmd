 ...
 */
function axtra_unset_default_widgets_args( $widget_areas ) {
...
add_filter( 'merlin_unset_default_widgets_args', 'axtra_unset_default_widgets_args' );
 ...
 */
function axtra_child_functions_php( $output, $slug ) {
...
add_filter( 'merlin_generate_child_functions_php', 'axtra_child_functions_php', 10, 2 );
 ...
 */
function axtra_local_import_files() {
	return array(
		array(
			'import_file_name'         => esc_html__('Main Demo', 'axtra'),
			...
			'local_import_redux'       => array(
				array(
					...
					'option_name' => 'axtra_options',
				),
			),
			'local_import_file'        => trailingslashit( get_template_directory() ) . 'demo-import/content/content.xml',
			'import_preview_image_url' => get_template_directory_uri() . '/screenshot.png',
			'import_notice'            => esc_html__( 'After you import this demo, you will have to setup the slider separately.', 'axtra' ),
			'preview_url'              => 'https://axtra.wealcoder.com/',
		),
	);
}

add_filter( 'merlin_import_files', 'axtra_local_import_files' );

/**
 * Execute custom code after the whole import has finished.
 */
function axtra_after_import_setup() {
	// Assign menus to their locations.
	$main_menu = get_term_by( 'name', 'Main Menu', 'nav_menu' );
	$sidebar_menu = get_term_by( 'name', 'Header Sitebar Menu', 'nav_menu' );
	...
	$front_page_id = get_page_by_title( 'Digital Maketing' );
...
add_action( 'merlin_after_all_import', 'axtra_after_import_setup' );