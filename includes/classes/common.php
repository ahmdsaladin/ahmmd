...
class Common {
	...
	function blog( $template = 'blog' ) {
		...
		if ( ( $wp_query->is_posts_page && 'blog' == $template ) || $template == 'page' || $template == 'archive' ) {
			$page_id = ( $wp_query->is_posts_page ) ? $wp_query->queried_object->ID : get_the_ID();
            
			$return = [
				'layout'            => axtra_meta( 'sidebar_sidebar_layout', $page_id, 'right' ),
				'sidebar_type'      => axtra_meta( 'sidebar_source_type', $page_id, 'd' ),
				'sidebar_elementor' => axtra_meta( 'sidebar_elementor_template', $page_id ),
				'sidebar'           => axtra_meta( 'sidebar_page_sidebar', $page_id, 'default-sidebar' ),
				'banner_type'       => axtra_meta( 'banner_source_type', $page_id ),
				'banner_elementor'  => axtra_meta( 'banner_elementor_template', $page_id ),
				'banner'            => axtra_set( axtra_meta( 'banner_page_background', $page_id ), 'url' ),
				'title'             => axtra_meta( 'banner_banner_title', $page_id ),
				'text'             => axtra_meta( 'banner_banner_text', $page_id ),
				'enable_banner'     => axtra_meta( 'banner_page_banner', $page_id ),
			];
		} else {
			...
			$return = [
				...
				'title'         => $options->get( $title ) ? $options->get( $title ) : axtra_the_title( $template ),
				'text'         => $options->get( $text ) ? $options->get( $text ) : axtra_the_title( $template ),
				'banner'        => axtra_set( $bg, 'url' ),
